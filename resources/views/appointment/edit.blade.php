<x-app>
    ...
    <div class="card shadow-lg p-3">
        ...
        <form action="{{ route('appointment.update', $appointment) }}" method="post" class="form">
            @csrf
            @method('put')
                ...
                <div class="col-md-12">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('patient_id') is-invalid  @enderror" id="patient_id"
                            ...
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}" @selected(old('patient_id', $appointment->patient_id) == $patient->id)>{{ $patient->name }} ({{ $patient->medical_record_number }})</option>
                            @endforeach
                        </select>
                        @error('patient_id')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('doctor_id') is-invalid  @enderror" id="doctor_id"
                            ...
                            @foreach($doctors as $doctor)
                                <option value="{{ $doctor->id }}" @selected(old('doctor_id', $appointment->doctor_id) == $doctor->id)>{{ $doctor->user->name }} - {{ $doctor->specialization }}</option>
                            @endforeach
                        </select>
                        @error('doctor_id')
                            <div class="invalid-feedback">
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('appointment_date') is-invalid  @enderror" type="datetime-local" id="appointment_date"
                            name="appointment_date" required value="{{ old('appointment_date', \Carbon\Carbon::parse($appointment->appointment_date)->format('Y-m-d\TH:i')) }}">
